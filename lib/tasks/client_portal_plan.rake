  ...
  PLAN_TASKS = [
    ...
    },
    {
      ...
      description: <<~DESCRIPTION.strip,
        ...
        Interface: expose a task detail view that can read and edit a multiline description, labels, comments, and checklist. Keep the content model ready for inline rich-media references without allowing untrusted HTML; B1 owns the actual board attachment upload/storage flow. Return only comment/checklist counts in board-card payloads and include full detail collections on the detail endpoint.
    ...
    },
    {
      ...
      description: <<~DESCRIPTION.strip,
        Schema: add one `brand_profile` per client account for display name, title, brokerage, office, phone, email, websites, bio, and social links for Instagram, Facebook, LinkedIn, YouTube, TikTok, X/Twitter, and future platforms. Add versioned `brand_assets` linked to media assets with slots such as primary logo, alternate logo, brokerage logo, headshot, and team logo; enforce one current asset per profile and slot with a partial unique index.
