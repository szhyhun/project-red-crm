class Api::V1::PricingPlansController < Api::V1::BaseController
  def index
    ...
    plans = policy_scope(PricingPlan).includes(:client_account, :user, :coupon, pricing_plan_prices: :product_variant).order(:name)
  ...
  def pricing_plan_params
    params.require(:pricing_plan).permit(:name, :client_account_id, :user_id, :coupon_id, :priority, :active,
                                         pricing_plan_prices_attributes: %i[id product_variant_id price_cents _destroy])
  end

  def serialize(plan)
    plan.slice(:id, :name, :client_account_id, :user_id, :coupon_id, :priority, :active).merge(
