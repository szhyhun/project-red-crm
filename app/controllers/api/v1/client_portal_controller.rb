class Api::V1::ClientPortalController < Api::V1::BaseController
  def show
    ...
    conversations = policy_scope(Conversation).includes(:listing).order(last_message_at: :desc, created_at: :desc)
  ...
  def serialize_listing(listing)
    {
      ...
      invoices: listing.invoices.order(created_at: :desc).map { |invoice| invoice.slice(:id, :number, :status, :currency, :total_cents, :balance_due_cents, :due_on, :sent_at) },
