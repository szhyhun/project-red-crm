class PricingPlan < ApplicationRecord
  ...
  # A plan belongs to one team or one person -- never to both, because then
  # nothing could say which price was promised.
  def exactly_one_owner
    owners = [ client_account_id, user_id ].count(&:present?)
    return if owners == 1

    errors.add(:base, "must belong to exactly one team or person")
  end
