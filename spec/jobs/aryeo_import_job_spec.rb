...
RSpec.describe AryeoImportJob do
  ...
  it "marks the run failed when the importer raises and re-raises the job error" do
    ...
    allow(importer).to receive(:call).and_raise(StandardError, "catalog database failure")
  ...
  it "does not replace a terminal completed-with-errors result with failed" do
    run.update!(status: :completed_with_errors, phase: "completed", completed_at: Time.current,
                error_details: [ "one record was invalid" ])
    importer = instance_double(Aryeo::Importer)
    allow(Aryeo::Importer).to receive(:new).and_return(importer)
    allow(importer).to receive(:call).and_raise(StandardError, "late notification failure")

    expect { described_class.perform_now(run.id) }.to raise_error(StandardError, "late notification failure")
